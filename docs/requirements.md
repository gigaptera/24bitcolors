...
全世界のユーザーの「好きな色」を収集し、統計分析してトレンドを可視化する Web アプリケーション。
**bit 診断方式**を採用し、ユーザーは 20 回の二択に答えるだけで、1677 万色（24bit）の中から真の好みの色を特定できる。
...
- **ユーザー体験**: パッと投票できる気軽さ（30 秒で完了）
- **データ収集**: 統計的に意味のある色の好みデータ
- **エンターテイメント性**: bit 診断の「当たる感」による驚きとシェア欲求
